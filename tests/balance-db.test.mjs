...
const walletSql = await readFile(new URL('../supabase/point-balances.sql', import.meta.url), 'utf8');
const balance = async () => (await rpc('wp_current_user')).balance;
test('wallet excludes historical completion and initializes old/new users to 2000', async () => {
  await identity(ids[1]); await answer(5, true); await identity(ids[2]); await answer(5, false);
  await db.exec('reset role'); await db.exec(walletSql);
  for (const id of ids) { await identity(id); assert.equal(await balance(), 2000); }
  await answer(5, false); assert.equal(await balance(), 2000);
  assert.equal((await snapshot()).balance_applied, false);
  await identity('55555555-5555-4555-8555-555555555555');
  assert.equal((await rpc('wp_register', ['新人', 'cat'])).balance, 2000);
});
test('settles once, resets only its own round, and preserves other rooms', async () => {
  await identity(ids[0]); await reset(5);
  await answer(6, true); assert.equal(await balance(), 2000);
  await identity(ids[1]); await answer(6, true);
  await identity(ids[2]); await answer(6, false); await answer(6, false);
  assert.equal(await balance(), 1899);
  await identity(ids[0]); assert.equal(await balance(), 2051);
  const other = await create(2000, '07:00', 2);
  await identity(ids[1]); await join(other);
  await rpc('wp_demo_submit', [other.room_code, 1, true]);
  await identity(ids[0]); await rpc('wp_demo_submit', [other.room_code, 1, false]);
  assert.equal(await balance(), 51);
  await reset(6); assert.equal(await balance(), 0);
  await assert.rejects(() => reset(6), /WP_DEMO_STALE/); assert.equal(await balance(), 0);
  await answer(7, false); await identity(ids[1]); await answer(7, false);
  await identity(ids[2]); await answer(7, false);
  await identity(ids[0]); assert.equal(await balance(), -101);
  const newRoom = await create(); assert.ok(newRoom.room_code);
  await identity(ids[1]); const invite = await create();
  await identity(ids[0]); await join(invite); assert.equal(await balance(), -101);
});
test('reapplication preserves balance and direct edits are denied', async () => {
  await db.exec('reset role'); await db.exec(walletSql);
  await identity(ids[0]); assert.equal(await balance(), -101);
  await assert.rejects(() => rpc('wp_balance', [ids[1]]), /permission denied/);
  await assert.rejects(() => db.exec('delete from public.wp_point_entries'), /permission denied/);
  await reset(7); assert.equal(await balance(), 0);
  for (const id of ids) { await identity(id); await answer(8, true); }
  await identity(ids[0]); assert.equal(await balance(), 0);
});