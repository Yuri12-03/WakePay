...
export default function DemoResults({ code }: { code: string }) {
  ...
  return <section className="wp-demo" aria-label="起床結果デモ">
    ...
    <h2>起きられましたか？</h2>
    <p className="wp-muted">各自の端末で結果を選ぶと、全員の回答後にポイントを分配します。実際の起床判定や残高には反映されません。</p>
    ...
    {data && <>
      ...
      <section className="wp-demo-result" aria-label="デモ結果">
      ...
      {data.can_reset ? <div className="wp-demo-reset">
        {confirmReset ? <><p>全員の回答と分配結果をリセットし、もう一度デモを始めますか？</p><button className="wp-secondary" disabled={busy} onClick={() => send('reset')}>全員の結果をリセットする</button><button className="wp-text-button" disabled={busy} onClick={() => setConfirmReset(false)}>やめる</button></> : <button className="wp-secondary" disabled={busy || !!error} onClick={() => setConfirmReset(true)}>結果をリセットする</button>}
